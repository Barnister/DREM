...
{
	int i_2;
	for(i_2=0;i_2<(cl->n);i_2++)
	{
		printf("|%d#%s,%s,%lld,%d|\n",cl->n,cl->s[i_2].name,cl->s[i_2].sex,cl->s[i_2].id,cl->s[i_2].age);
	}
}

void pop_student(struct class_t* cl)
{
	int i_1,index_0 = fin_student(cl);
	printf("确认是否删除该学生(1、是 2、否)\n：");
	scanf("%d",&i_1);
	if(i_1==1){
		strcpy_s(cl->s[index_0].name,strlen(cl->s[index_0].name) + 1,cl->s[index_0+1].name);
		strcpy_s(cl->s[index_0].sex,strlen(cl->s[index_0].sex) + 1,cl->s[index_0+1].sex);
		cl->s[index_0].age = cl->s[index_0+1].age;
		cl->s[index_0].id = cl->s[index_0+1].id;
		printf("删除完成\n");}
	else{
		printf("STOP");}
